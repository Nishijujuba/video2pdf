...
def make_quick_build_dir(video_output_dir: Path, build_root: Path | None = None) -> Path:
    now = _dt.datetime.now()
    run_id = f"{now.strftime('%Y%m%d_%H%M%S_%f')}_{uuid.uuid4().hex[:8]}"
    root = build_root.resolve() if build_root is not None else video_output_dir / TRASH_DIR_NAME / "latex-build"
...
def compile_quick(
    ...
    *,
    ...
    build_dir = make_quick_build_dir(video_output_dir, build_root=build_root)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--build-root", type=Path, help="Optional short temporary build root for quick/final modes.")
