...
Global Gate status is `active_global_gate`. The Acceptance Reviewer is an independent read-only visual-quality actor working from a provider-created Task Envelope. Delivery Quality policy, Role Projections, the immutable input binding, and the Acceptance Report v2 Skeleton define the allowed evidence and decision ownership. For newly generated Bilibili and YouTube work, `start-run --project-config` validates the Workflow Release Profile, committed Cutover Authority Tombstone, and current Profile activation before creating a Run. Bilibili and YouTube are `active_kernel` for new tasks. Existing Legacy directories remain on explicit Legacy maintenance routes. Platform Kernel authority remains unchanged by this acceptance gate.
...
2. For a Kernel input, first complete provider-owned delivery readiness and rendered-page evidence so the real Run is at `ready_for_delivery`. Use the current committed final-quality input binding from that exact Run and Control Store revision. `acceptance-prepare` fails before reviewer dispatch when this revision cannot be bound directly to `accepted`.
3. Run `acceptance-prepare` to create the sole Acceptance Execution Context, fixed Skeleton, one Visual Quality Task Envelope, its Claim, and one provider-created Attempt with an exact authorized read set.
...
7. For a passing Kernel report, run `delivery-acceptance-bind`; this one recoverable operation binds the canonical report and atomically advances `ready_for_delivery -> accepted`. Run a fresh `delivery_guard.py check` only after `accepted`, then run the guarded `accepted -> delivered` lifecycle transition. A successful path uses this one Acceptance Execution and one Reviewer Judgment Patch.
...
The video-level target binds the final PDF, main TeX file, `review/acceptance/allowed_artifacts_manifest.json`, `review/acceptance/acceptance_report.json`, and `review/acceptance/delivery_guard_report.json`. Historical already-compiled Legacy PDFs may retain `latex_compile_report.v1` provenance at `review\latex\compile_report.json`; that historical contract binds current TeX/PDF fingerprints plus the guarded wrapper producer, wrapper contract, wrapper mode, wrapper script fingerprint, and final-mode invocation arguments. A newly guarded Legacy compile uses Registry-valid `final-compile-report/1.0.0` provenance and binds its Acceptance-authorized Final Compile Manifest, manifest-declared `main.tex` entrypoint, current adapter, recorder, runtime, generated-input closure, and final PDF. The target must record `attempt_limit: 3`.
...
The Acceptance Reviewer evaluates postcompile visual quality from the Task Envelope exact `authorized_read_set`. The target's declared compile-report path supplies either historical `latex_compile_report.v1` or modern `final-compile-report/1.0.0` provenance to `delivery_guard.py check`. A compile report cannot replace acceptance_report.json, cannot override `overall_status`, and cannot serve as Acceptance Reviewer quality judgment.
