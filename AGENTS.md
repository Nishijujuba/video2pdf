...
The Workflow Release Profile is the sole ordinary-admission authority for completed Workflow 2.0 capabilities. Bilibili and YouTube are `active_kernel` for new tasks, and Batch is `active_batch` for new batches. New single-video work enters through `start-run --project-config`; new Batch work enters through `batch-plan --project-config`. Both routes require the committed Cutover Authority Tombstone and the current Profile activation. Existing Bilibili, YouTube, and Batch directories remain on their explicit Legacy maintenance routes. The shared final-quality authority remains the `active_global_gate`; Acceptance Report v2, every-page visual review, Delivery Guard, and the delivery lifecycle remain unchanged. `CONTEXT-MAP.md`, active context glossaries, and the decision map record this boundary explicitly.
