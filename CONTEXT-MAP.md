...
The Workflow Release Profile is the sole ordinary-admission authority for completed Workflow 2.0 capabilities. Bilibili and YouTube are `active_kernel` for new tasks, and Batch is `active_batch` for new batches. `start-run --project-config` owns new single-video admission; `batch-plan --project-config` owns new Batch admission. Both routes require the committed Cutover Authority Tombstone and current Profile activation. Pre-existing Bilibili, YouTube, and Batch directories retain explicit Legacy maintenance. Delivery Quality and Acceptance Report v2 remain the shared `active_global_gate` for every input track.
