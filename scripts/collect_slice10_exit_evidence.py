...
EVIDENCE_DIR = PROJECT_ROOT / "evidence/slice-10"
LOG_DIR = EVIDENCE_DIR / "logs"
MANIFEST_PATH = EVIDENCE_DIR / "exit-evidence-manifest.json"
REFRESH_ROOT = PROJECT_ROOT / "待删除/exit-evidence-refresh/slice-10"
...
def main() -> int:
    if git("status", "--porcelain=v1", "--untracked-files=all"):
        print("ERROR: Slice 10 evidence collection requires a clean implementation HEAD", file=sys.stderr)
        return 2
    ...
    fixtures = [{"role": role, "path": path, "sha256": sha256_file(PROJECT_ROOT / path)} for role, path in FIXTURE_SPECS]
