...
EVIDENCE_DIR = PROJECT_ROOT / "evidence/slice-08"
LOG_DIR = EVIDENCE_DIR / "logs"
MANIFEST_PATH = EVIDENCE_DIR / "exit-evidence-manifest.json"
REFRESH_ROOT = PROJECT_ROOT / "待删除/exit-evidence-refresh/slice-08"
...
def preserve_previous_evidence() -> None:
    existing = [
        path
        for path in [MANIFEST_PATH, *LOG_DIR.glob("*.log")]
        if path.exists()
    ]
    if not existing:
        return
    destination = REFRESH_ROOT / datetime.now(timezone.utc).strftime(
        "%Y%m%d_%H%M%S_%f"
    )
...
def main() -> int:
    if git("status", "--porcelain=v1", "--untracked-files=all"):
        print(
            "ERROR: Slice 8 evidence collection requires a clean implementation HEAD",
            file=sys.stderr,
        )
        return 2
