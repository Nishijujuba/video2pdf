...
from .utils import read_json, require_safe_path_segment, sha256_file
...
class OrdinaryRunStartup:
    ...
    def start(
        ...
        *,
        ...
        profile = ReleaseMaintenance(self.runtime_project_root).require_for_admission(
        ...
        activation = self._require_activation_fence(
            paths["release_profile"].parent
        ...
        request_id = hashlib.sha256(
            "\0".join((str(config_path), platform, source_url, session_id)).encode(
                "utf-8"
            )
        ).hexdigest()[:32]
        probe = self._find_probe(kernel, request_id=request_id, platform=platform)
        if probe is None:
            task_start = (
        ...
        initialized = kernel.initialize_production_source(
            ...
            global_gate_binding={
                "authority_path": str(paths["release_profile"]),
                "authority_sha256": activation["profile_sha256"],
    ...
    def _require_activation_fence(
        ...
        *,
        ...
        try:
            self.contracts.validate("workflow-admission-activation", value)
        ...
        if (
            value.get("release_id") != profile["release_id"]
            or value.get("profile_sha256") != profile_sha
        ):
            _reject(
