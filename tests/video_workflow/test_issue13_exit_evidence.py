...
from scripts import collect_issue13_exit_evidence as collector
from scripts import issue13_exit_evidence_contract as contract
...
class Issue13ExitEvidenceTests(unittest.TestCase):
    def manifest(self) -> dict:
        ...
        return {
            ...
            "evidence_paths": ["evidence/slice-12/exit-evidence-manifest.json", "evidence/slice-12/logs/tests.log"],
            ...
            "guarded_delivery_evidence": {
                "collection": {
                    ...
                    "path": "evidence/slice-12/guarded-delivery/collection.json",
                    "sha256": sha,
                },
                ...
                "artifacts": [
                    {
                        "role": role,
                        "path": f"evidence/slice-12/guarded-delivery/{role}.json",
                        "sha256": sha,
                    }
                    for role in (
                        "run_record",
                ...
                "qualification_run": {
                    ...
                    "command_record": {
                        ...
                        "path": "evidence/slice-12/guarded-delivery/qualification/command.json",
                        "sha256": sha,
                    },
                    "terminal_status": {
                        "role": "persisted_terminal_status",
                        "path": "evidence/slice-12/guarded-delivery/qualification/status.json",
                        "sha256": sha,
                    },
                    "exit_code": {
                        "role": "persisted_exit_code",
                        "path": "evidence/slice-12/guarded-delivery/qualification/exit-code.txt",
                        "sha256": sha,
                    },
                },
            },
            ...
            "commands": [
                {"test_id": command_id, "command": list(command), "expected_exit_code": exit_code, "actual_exit_code": exit_code, "log": {"role": "command_log", "path": f"evidence/slice-12/logs/{command_id}.log", "sha256": sha}, "conforms": True}
            ...
            "artifact_fingerprints": [{"role": "implementation_artifact", "path": "scripts/issue13_exit_evidence_contract.py", "sha256": sha}],
            "unresolved_exceptions": [],
            "overall_decision": "pass",
        }

    def test_bilibili_kernel_cutover_pass(self) -> None:
        fixture = json.loads((ROOT / "tests/video_workflow/fixtures/exit_evidence/slice12.valid.json").read_text(encoding="utf-8"))
        ...
        validator.validate_issue13_cutover(self.manifest())

    def test_youtube_legacy_preserved(self) -> None:
        self.assertEqual({"bilibili": "active_kernel", "youtube": "active_legacy"}, self.manifest()["platform_statuses"])

    def test_youtube_authority_change_rejected(self) -> None:
        # scenario_id=youtube_kernel_activated; target_invariant=platform_statuses;
        # mutation_seam=platform_statuses.youtube; rematerialized_nodes=none;
        # intentionally_stale_nodes=none; scenario_class=single_contradiction.
        scenario = json.loads((ROOT / "tests/video_workflow/fixtures/exit_evidence/slice12.youtube-kernel.invalid.json").read_text(encoding="utf-8"))
    ...
    def _isolated_delivered_bilibili_fixture(self) -> tuple[Path, Path, Path]:
        project = new_case_dir(self.id(), label="issue13-exit-evidence")
        run_dir = project / "workspace" / "Issue 13 Bilibili Run_20260809_090000"
        review_dir = run_dir / "review" / "acceptance"
        workflow_dir = run_dir / "workflow"
        source_dir = run_dir / "source"
        session_id = "session-issue13-exit-evidence"
        run_id = "13131313131313131313131313131313"

        final_pdf = run_dir / "Issue 13 Bilibili Run.pdf"
        final_pdf.parent.mkdir(parents=True, exist_ok=True)
        final_pdf.write_bytes(b"%PDF-1.7\n% isolated issue13 fixture\n")
        ...
        _write_json(
            workflow_dir / "run.json",
            {
                ...
                "run_id": run_id,
                "request_id": "issue-13-exit-evidence",
                ...
                "normalized_title": "Issue 13 Bilibili Run",
                ...
                "source_blocker": None,
                "phase": "delivered",
                "initialization_intent_id": "initialize-issue-13-exit-evidence",
                "coordination_revision": 4,
                "last_mutation_intent_id": "a" * 64,
                "artifact_plan": "workflow/artifact-plan.json",
                "artifact_generations": {
                    "source_manifest": {"path": "source/source-manifest.json", "generation": 1, "sha256": _sha256(source_manifest), "producer": "kernel:source-acquisition", "committed_at": "2026-08-09T01:00:00Z", "source_epoch": 1},
                    "final_pdf": {"path": final_pdf.name, "generation": 1, "sha256": _sha256(final_pdf), "producer": "kernel:final-compile", "committed_at": "2026-08-09T02:00:00Z", "source_epoch": 1},
                },
                "checkpoint_dependencies": {"source_ready": [], "final_quality_ready": ["source_ready"]},
                "checkpoints": {
                    "source_ready": {"status": "current", "artifact_bindings": [{"logical_id": "source_manifest", "generation": 1, "sha256": _sha256(source_manifest)}], "prerequisite_bindings": [], "evidence_sha256": _sha256(source_manifest), "completed_at": "2026-08-09T01:00:00Z"},
                    "final_quality_ready": {"status": "current", "artifact_bindings": [{"logical_id": "final_pdf", "generation": 1, "sha256": _sha256(final_pdf)}], "prerequisite_bindings": [{"name": "source_ready", "evidence_sha256": _sha256(source_manifest)}], "evidence_sha256": _sha256(guard_report), "completed_at": "2026-08-09T02:00:00Z"},
                },
                "delivery": {
                    "stage": "delivered",
                    "ownership": {"session_id": session_id, "generation": 1},
        ...
        qualification_run_id = "13131313-1313-4313-8313-131313131313"
        qualification_argv = [sys.executable, "-X", "utf8", "-B", "-m", "unittest", "-v", "tests.video_workflow.test_issue13_exit_evidence"]
        _write_json(
    ...
    def test_public_cli_collects_real_delivered_run_and_finalizes_schema_valid_manifest(self) -> None:
        run_dir, current_target, qualification_run = self._isolated_delivered_bilibili_fixture()
        ...
        collected = subprocess.run(
            [
                ...
                "scripts.collect_issue13_exit_evidence", "collect",
                "--run-dir", str(run_dir),
        ...
        self.assertEqual("issue13-exit-evidence-collection", collection["schema_name"])
        self.assertEqual("13131313131313131313131313131313", collection["run_id"])
        self.assertEqual(
            {
        ...
        finalized = subprocess.run(
            [
                sys.executable, "-X", "utf8", "-B", "-m",
                "scripts.collect_issue13_exit_evidence", "finalize",
    ...
    def test_collect_rejects_succeeded_run_outside_closed_issue13_qualification(self) -> None:
        mutations = {
    ...
    def test_finalize_fingerprints_complete_implementation_change_set(self) -> None:
        repository = new_case_dir(self.id(), label="issue13-fingerprint-repository")
        ...
        git("init")
        git("config", "user.email", "issue13-tests@example.invalid")
        git("config", "user.name", "Issue 13 Tests")
        for _role, relative in contract.FIXTURE_SPECS:
            path = repository / relative
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text("{}\n", encoding="utf-8")
        contract_path = repository / "scripts" / "issue13_exit_evidence_contract.py"
        ...
        feature_path.write_text("ACTIVE_PLATFORM = 'bilibili'\n", encoding="utf-8")
    ...
    def test_platform_reconcile_rejects_evidence_or_global_gate_drift(self) -> None:
        ...
        for drift in ("exit_evidence", "global_gate"):
            with self.subTest(drift=drift):
                helper = Issue13PlatformCutoverTests(methodName="test_exact_bilibili_activation_retry_is_idempotent")
                ...
                interrupted = run_platform_cli(
                    ...
                    "--activated-at", "2026-08-09T00:00:00Z",
    ...
    def test_platform_activation_rejects_self_declared_guarded_delivery(self) -> None:
        ...
        scenario = json.loads(
            (ROOT / "tests/video_workflow/fixtures/exit_evidence/slice12.self-declared.invalid.json").read_text(encoding="utf-8")
        )
        invalid = self.manifest()
